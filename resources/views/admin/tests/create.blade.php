...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.test.store') }}">
        ...
        <div class="form-group">
            <label for="title" class="col-form-label">Название теста</label>
        ...
        <div class="form-group">
            ...
            <select id="grade" class="form-control{{ $errors->has('grade') ? ' is-invalid' : '' }}" name="grade">
                ...
                @foreach (\App\Entity\User\User::gradeList() as $gradeId => $grade)
                    <option value="{{ $gradeId }}" {{ $gradeId == old('grade') ? ' selected' : '' }}>
                        {{ $grade }}
                    </option>
                @endforeach
            </select>
        ...
        <div class="form-group">
            ...
            <input id="duration" class="form-control{{ $errors->has('duration') ? ' is-invalid' : '' }}" name="duration" value="{{ old('duration') }}" type="number" min="10" max="180" required>
